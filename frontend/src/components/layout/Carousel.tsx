...
export const Carousel: React.FC<CarouselProps> = ({
    ...
    const getImageStyles = (index: number): { className: string, opacity: number, cursor: 'pointer' | 'default' } => {
    // We explicitly define the transition property in the class for reliability
    const baseClasses = "absolute top-0 left-0 w-full h-full object-cover rounded-lg shadow-xl transition-all duration-500 ease-in-out";
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto rounded-lg aspect-[4/3] perspective-100"
    ...
    >
      ...
      <div className="relative w-full h-full">
        {images.map((image, index) => {
            const { className, opacity, cursor } = getImageStyles(index);
