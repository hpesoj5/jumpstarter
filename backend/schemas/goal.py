...
class GoalPrerequisites(BaseModel):
    ...
    status: Literal['data_extracted'] = 'data_extracted'
...
class Definitions(DefinitionsCreate):
    id: int
    prerequisites: Optional[GoalPrerequisites] = None # Includes the data expected from Phase 2
...
class DailyTaskBase(BaseModel):
    """Base model for a single actionable daily task."""
    description: str = Field(
        description="A specific, measurable, and actionable task description (e.g., 'Practice the first 5 chords of C-major scale')."
    )
    dailies_date: date = Field(
        description="The estimated starting date for this daily, formatted as YYYY-MM-DD."
    ),
    start_time: time = Field(
        description="The estimated starting time for this phase, formatted as HH:MM:00."
    ),
    time_needed_minutes: int = Field(
        description="The estimated time required to complete the task, in minutes."
    )
    suggested_resource: List[str] = Field(
        description="Specific tools, links, or resources to help complete the task (e.g., 'YouTube tutorial link' or 'Chapter 3 of assigned textbook')."
    )

class DailyTaskCreate(DailyTaskBase):
    pass # Used for generation/POST body

class DailyTaskGeneration(BaseModel):
    """Container for the weekly schedule of generated daily plans."""
    status: Literal['daily_tasks_generated'] = 'daily_tasks_generated'
    weekly_plan: List[DailyTaskBase] = Field(
        description="A list containing the daily task schedule for the duration of the next 7 days, or until phase end."
    )

class PhaseDailies(BaseModel):
    """Container for the schedule of all generated daily plans."""
    dailies: List[DailyTaskBase] = Field(
        description="A list containing the daily task schedule for the current phase."
    )

class GoalConfirm(BaseModel):
    user_id: int
    data: Union[DefinitionsBase, GoalPrerequisites, PhaseGeneration]